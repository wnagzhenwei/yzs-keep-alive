...
const props = withDefaults(defineProps<YzsKeepAliveProps>(), {
  include: undefined,
  exclude: undefined,
  max: 10
})
...
const emit = defineEmits<{
  activated: [component: Component]
  deactivated: [component: Component]
}>()
...
// 当前组件实例
const currentInstance = shallowRef<any>(null)

  // 组件引用
  const componentRef = ref<any>(null)
  // 直接捕获实例的辅助ref
  const internalInstanceRef = ref<any>(null)

// 存储容器（用于隐藏DOM元素）
const storageContainer = ref<HTMLElement | null>(null)

// 从slot提取组件信息
const extractComponentFromSlot = (): { component: Component, props: Record<string, any>, key: string } | null => {
  ...
  return {
    ...
    props: vnode.props || {},
    key: vnode.key as string || generateKey(vnode.type as Component, vnode.props || {})
  }
}
...
// 检查组件是否应该被缓存
const shouldCacheComponent = (component: Component): boolean => {
    ...
    if (cached) {
      ...
      if (cached.instance && cached.data) {
        ...
        if (cached.container && cached.instance.vnode.el) {
          const el = cached.instance.vnode.el as HTMLElement
          if (el.parentNode === cached.container) {
            // 将DOM元素移回原来的位置
            const parentEl = getCurrentInstance()?.vnode.el as HTMLElement
            if (parentEl) {
...
watch(() => props.max, (newMax) => {
  // 更新缓存管理器的最大容量
  if (cacheManager && typeof (cacheManager as any).setMaxSize === 'function') {
    ;(cacheManager as any).setMaxSize(newMax)
  }
})
