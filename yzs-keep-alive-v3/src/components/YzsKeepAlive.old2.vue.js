...
import { getCurrentInstance, cloneVNode, isVNode, watch, onMounted, onUpdated, onBeforeUnmount, useSlots } from 'vue';
...
sharedContext.activate = (vnode, container, anchor, isSVG, optimized) => {
    ...
    queuePostRenderEffect(() => {
        ...
        if (componentInstance.a) { // activated hooks
            ;
            componentInstance.a.forEach(fn => fn());
        }
...
sharedContext.deactivate = (vnode) => {
    ...
    queuePostRenderEffect(() => {
        if (componentInstance.da) { // deactivated hooks
            ;
            componentInstance.da.forEach(fn => fn());
        }
