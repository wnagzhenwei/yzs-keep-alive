...
import type { ComponentState, InstanceManager } from '../types'
import { deepClone, safeCloneState } from './utils'
...
export class InstanceManagerImpl implements InstanceManager {
  // 存储组件状态
  private stateCache = new Map<number, ComponentState>()
  // 存储组件实例引用
  private instanceCache = new Map<number, ComponentInternalInstance>()
  // 存储包装后的组件
  private wrappedComponents = new Map<Component, Component>()
  ...
  saveInstanceState(instance: ComponentInternalInstance): ComponentState {
    try {
      ...
      const simpleState: Record<string, any> = {}
   ...
   * 清理实例缓存
   */
  cleanupInstance(uid: number): void {
   ...
   * 获取所有缓存的实例
   */
  getCachedInstances(): Map<number, ComponentInternalInstance> {
 ...
 * 创建实例管理器
 */
export function createInstanceManager(): InstanceManager {
